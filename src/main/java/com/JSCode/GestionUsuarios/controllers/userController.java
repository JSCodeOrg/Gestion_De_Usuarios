...
public class UserController {
    ...
    public ResponseEntity<ApiResponse<Void>> createNewUser(@RequestBody WorkerRegisterDto workerData){
        if(workerData.getEmail() == null || workerData.getRole_id() == null){
            return ResponseEntity.badRequest().body(new ApiResponse<>("Email y rol necesarios", null, true, 400));
        }
        userService.createWorker(workerData.getEmail(), workerData.getRole_id());

        return ResponseEntity.ok(new ApiResponse<>("Usuario creado exitosamente", null, false,  200));
    }
