...
import com.JSCode.GestionUsuarios.model.*;
import com.JSCode.GestionUsuarios.repository.*;
import com.JSCode.GestionUsuarios.security.JwtUtil;
import com.JSCode.GestionUsuarios.service.*;
...
public class UserServiceTest {
    ...
    void getUserAddress_returnsCorrectAddress() {
        ...
        addressDTO.setDireccion("Cra 1 #2-3");
        addressDTO.setCiudad("Bogotá");
        person.setDireccion(addressDTO.getDireccion());
        when(userRepository.findById(1L)).thenReturn(Optional.of(user));
        when(personRepository.findByUser(user)).thenReturn(Optional.of(person));
        AddressDTO result = userService.getUserAddress(1L);
        assertEquals("Cra 1 #2-3", result.getDireccion());
    }
    ...
    void checkRecoveryCode_validCodeReturnsResponse() {
        ...
        assertEquals("rec-token", response.getRecoveryToken());
    }
}
